...
async function testConnection() {
  ...
  try {
    ...
    console.log('   3. ネットワークの問題（ファイアウォール、セキュリティグループなど）')
    console.log('   4. Supabaseプロジェクトが停止している')
    console.log('   5. IPアドレスがSupabaseの許可リストに含まれていない')
    console.log('')
    console.log('確認方法:')
    console.log('   - .envファイルのDATABASE_URLを確認')
    console.log('   - Supabaseダッシュボードでプロジェクトの状態を確認')
    console.log('   - ネットワーク接続テスト: node scripts/test-network-connection.js')
    console.log('   - SupabaseのSettings > Database > Connection Poolingで接続文字列を確認')
  } finally {
