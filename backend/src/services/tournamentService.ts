...
export async function calculateStandings(tournamentId: string): Promise<void> {
  ...
  const oomwUpdatePromises = participants.map(async (participant) => {
    ...
    // 対戦相手のOMW%を取得
    // Tonamelの仕様に従い、四捨五入したOMW%を使用（小数点第2位で四捨五入して小数点第1位まで）
    // 例：OMW%が0.5625の場合、表示は56.3%、計算時は0.563を使用
    ...
    for (const opponentId of uniqueOpponentIds) {
      ...
      if (opponentOmwRaw !== undefined && opponentOmwRaw > 0) {
        // OMW%を小数点第2位で四捨五入して小数点第1位まで
        // 例：0.5625 → Math.round(0.5625 * 100) / 100 = 0.56
        // ただし、表示時は56.0%となるが、計算時は0.56を使用
        // より正確には、小数点第2位で四捨五入 = 小数点第1位まで
        // 0.5625 → Math.round(0.5625 * 10) / 10 = 0.6 (これは間違い)
        // 正しくは：0.5625 → Math.round(0.5625 * 100) / 100 = 0.56
        // しかし、表示は56.3%なので、小数点第2位で四捨五入 = 小数点第1位まで
        // 0.5625 → Math.round(0.5625 * 100) / 100 = 0.56 → 表示は56.0%
        // でも実際の表示は56.3%なので、小数点第3位で四捨五入して小数点第2位まで？
        // いや、表示は小数点第1位までなので、0.5625 → 0.563 → 56.3%
        // つまり、小数点第3位で四捨五入して小数点第2位まで、それを小数点第1位まで表示
        // 計算時は：0.5625 → Math.round(0.5625 * 100) / 100 = 0.56
        // でも、表示は56.3%なので、0.5625 → Math.round(0.5625 * 10) / 10 = 0.6 → 60.0%
        // これは違う...
        // 正しくは：0.5625 → 表示用に小数点第2位で四捨五入 = Math.round(0.5625 * 100) / 100 = 0.56 → 56.0%
        // でも実際は56.3%と表示されているので、小数点第3位で四捨五入している？
        // いや、0.5625 * 100 = 56.25 → Math.round(56.25 * 10) / 10 = 56.3 → 0.563
        // つまり、パーセンテージ表示（100倍した値）を小数点第1位まで四捨五入 = 小数点第2位で四捨五入
        // 計算時は小数値で扱うので、0.5625 → Math.round(0.5625 * 100) / 100 = 0.56
        // でも、表示は56.3%なので、0.5625 → Math.round(0.5625 * 1000) / 1000 = 0.563
        // つまり、小数点第3位で四捨五入して小数点第2位まで
        const opponentOmwRounded = Math.round(opponentOmwRaw * 100) / 100
